...
const f = "float PI = 3.141592653589793238;",
  ...
  G = {
    ...
    "morph-y": {
      uniforms: { intensity: { value: 0.3, type: "f", min: 0, max: 2 } },
    ...
    shutters: {
      uniforms: { intensity: { value: 50, type: "f", min: 1, max: 100 } },
      fragment: `\n    ${U}\n    uniform float intensity;\n    mat2 rotate(float a) {\n      float s = sin(a);\n      float c = cos(a);\n      return mat2(c, -s, s, c);\n    }\n    const float PI = 3.1415;\n    const float angle1 = PI *0.25;\n    const float angle2 = PI *0.25;\n\n    void main()\t{\n      vec2 newUV = (vUv - vec2(0.5))*resolution.zw + vec2(0.5);\n\n      vec2 uvDivided = fract(newUV*vec2(intensity,1.));\n\n      vec2 uvDisplaced1 = newUV + rotate(angle1)*uvDivided*progress*0.1;\n      vec2 uvDisplaced2 = newUV + rotate(angle2)*uvDivided*(1. - progress)*0.1;\n\n      vec4 t1 = texture2D(texture1,uvDisplaced1);\n      vec4 t2 = texture2D(texture2,uvDisplaced2);\n\n      gl_FragColor = mix(t1, t2, progress);\n    }\n\n  `,
    },
    slices: {
      uniforms: { size: { value: 0.25, type: "f", min: 0.1, max: 1 } },
...
class K {
  constructor(e) {
    ...
    (this.shader = n),
      ...
      (this.renderer = new l({ dpr: 2, webgl: 2, alpha: !0 })),
      ...
      (this.camera = new c(this.gl, { fov: 45 })),
  ...
  animateUniform(e, n, t) {
    ...
    const r = n > e.value ? "next" : "prev",
      a = (e, n) => ("next" === r && e >= n) || ("prev" === r && e <= n),
      l = () => {
        if (this.destroyed) return;
        (i = new Date().getTime()), null === s && (s = i);
        const r = Math.max(Math.min((i - s) / this.swiper.params.speed, 1), 0),
          c = 0.5 - Math.cos(r * Math.PI) / 2;
        let m = o + c * (n - o);
        if ((a(m, n) && (m = n), (e.value = m), a(m, n)))
          return cancelAnimationFrame(this.animateUniformFrame), void (t && t());
  ...
  init(e) {
    ...
    this.images.forEach((e, o) => {
      const i = new Promise((n) => {
        ...
        const s = new m(this.gl);
        (i.onload = () => {
          (s.image = i), (t.textures[o] = s), n();
        }),
          (i.src = e);
      });
      n.push(i);
  ...
  resize() {
    ...
    if (
      (this.camera.perspective({ aspect: e / n, fov: (180 / Math.PI) * 2 * Math.atan(1 / (2 * t)) }),
      !this.textures[0].image)
    )
      return;
    const o = this.textures[0].image.height / this.textures[0].image.width;
    let i, s;
    n / e > o ? ((i = (e / n) * o), (s = 1)) : ((i = 1), (s = n / e / o)),
      ...
      (this.material.uniforms.resolution.value.w = s),
      this.shader.vertex &&
        this.vertexMaterial &&
        ((this.vertexMaterial.uniforms.resolution.value.x = e),
        (this.vertexMaterial.uniforms.resolution.value.y = n),
        (this.vertexMaterial.uniforms.resolution.value.z = i),
        (this.vertexMaterial.uniforms.resolution.value.w = s)),
      this.shader.vertex
        ? (this.nextMesh.scale.set(this.camera.aspect / 2, 0.5, 0.5),
          this.currentMesh.scale.set(this.camera.aspect / 2, 0.5, 0.5))
        : ((this.plane.scale.x = this.camera.aspect), (this.plane.scale.y = 1));
  }
  createMaterial() {
    return new h(
      this.gl,
      ((e, s) => {
        for (var r in s || (s = {})) t.call(s, r) && i(e, r, s[r]);
        if (n) for (var r of n(s)) o.call(s, r) && i(e, r, s[r]);
        return e;
      })(
        {
          ...
          uniforms: {
            ...
            displacement: { type: "f", value: this.displacementTexture },
            resolution: { type: "v4", value: new p() },
          },
          vertex: this.shader.vertex || C,
          fragment: this.shader.fragment,
        },
        this.shader.vertex ? { transparent: !0, depthWrite: !1 } : {}
      )
    );
  }
  addObjects() {
    if (
      ...
      const e = (function (e, n, t) {
          ...
          for (let r = 0; r < a; r += 1) {
            ...
            for (let e = 0; e <= o; e += 1) {
              ...
              c.push(e * i + d - 1, v - 1, 0),
                x.push((e * i + d) / 2, v / 2),
                c.push(e * i + i / 2 + d - 1, s + v - 1, 0),
                x.push((e * i + i / 2 + d) / 2, (s + v) / 2),
                c.push(e * i - i / 2 + d - 1, s + v - 1, 0),
                x.push((e * i - i / 2 + d) / 2, (s + v) / 2);
              let f = w(e / a, r / a) + Math.random();
              const U = y(v / 2 + (2 * f) / n);
              let E = Math.random();
              l.push(U, y(U + 0.1 * t), y(U + 0.1 * t)), g.push(E, E, E);
              const F = [e * i + d - 1, v - 1, 0];
              m.push(...F, ...F, ...F);
              const G = [2 * o * M(-0.3, 0.3), -2 * M(-0.3, 0.3) * 1.5, -A(0.5)],
                C = [2 * o * M(0.3, 0.6), -2 * M(0.3, 0.6) * 1.5, -A(0.5)];
              h.push(...G, ...G, ...G),
                p.push(...C, ...C, ...C),
                ...
                c.push(e * i + i / 2 + d - 1, s + v - 1, 0),
                x.push((e * i + i / 2 + d) / 2, (s + v) / 2),
                (f = w((e + 1) / a, r / a) + Math.random());
              const R = y(v / 2 + (2 * f) / n);
              (E = Math.random()), l.push(R, R, y(R + 0.1 * t)), g.push(E, E, E);
          ...
          const f = new r(e);
          return (
            f.addAttribute("position", { size: 3, data: new Float32Array(c) }),
            f.addAttribute("bary", { size: 3, data: new Float32Array(u) }),
            f.addAttribute("uv", { size: 2, data: new Float32Array(x) }),
            f.addAttribute("offset", { size: 1, data: new Float32Array(l) }),
            f.addAttribute("centroid1", { size: 3, data: new Float32Array(m) }),
            f.addAttribute("control0", { size: 3, data: new Float32Array(h) }),
            f.addAttribute("control1", { size: 3, data: new Float32Array(p) }),
            f.addAttribute("random", { size: 1, data: new Float32Array(g) }),
            f
          );
      ...
      (this.vertexMaterial = this.createMaterial()),
        ...
        (this.currentMesh = new g(this.gl, { geometry: e, program: this.material })),
        (this.nextMesh = new g(this.gl, { geometry: e, program: this.vertexMaterial })),
        (this.nextMesh.position.z = -1e-4),
        this.currentMesh.setParent(this.scene),
        this.nextMesh.setParent(this.scene);
    } else {
      const e = new x(this.gl, { width: 1, height: 1, widthSegments: 2, heightSegments: 2 });
      (this.plane = new g(this.gl, { geometry: e, program: this.material })), this.plane.setParent(this.scene);
    }
  }
  replaceShader(e) {
    let n, t;
    this.shader.vertex
      ? ((n = this.material.uniforms.texture1.value), (t = this.vertexMaterial.uniforms.texture1.value))
      : ((n = this.material.uniforms.texture1.value), (t = this.material.uniforms.texture2.value));
    const o = "random" === e || Array.isArray(e) ? R(e) : G[e],
      { fragment: i, uniforms: s, vertex: r } = o;
    (this.shader = o),
      (this.vertex = r || C),
      (this.fragment = i || ""),
      (this.uniforms = s || {}),
      this.addObjects(),
      this.shader.vertex
        ? ((this.material.uniforms.texture1.value = t), (this.vertexMaterial.uniforms.texture1.value = t))
        : ((this.material.uniforms.texture1.value = n),
  ...
  setProgress(e, n, t, o) {
    ...
    this.swiper.params.loop &&
      ((e = parseInt(this.swiper.slides[e].getAttribute("data-swiper-slide-index"), 10)),
    ...
    (this.material.uniforms.texture1.value = s),
      ...
      o
        ? (0 === t && 0 === this.material.uniforms.progress.value && (this.material.uniforms.progress.value = 1),
          ...
          this.animateUniform(this.material.uniforms.progress, t, () => {
            ("random" === this.swiper.params.gl.shader || Array.isArray(this.swiper.params.gl.shader)) &&
...
const Y = new d(".swiper", {
    modules: [
      u,
      function ({ swiper: e, on: n, extendParams: t }) {
        e.gl = null;
        let o = !1;
        t({ gl: { shader: "random", displacementMap: void 0 } });
        const i = () => {
          e.gl = new K({ debug: !0, swiper: e, shader: e.params.gl.shader });
        };
        let s, r;
        n("beforeInit", () => {
          ...
          n("setTranslate", () => {
            ...
            e.gl || i(),
              e.slides.forEach((o, i) => {
                const s = o.progress;
                ((s > 0 && s < 1) || (0 === s && e.progress < r)) && ((n = i), (t = i + 1), (a = s)),
                  ((s < 0 && s > -1) || (0 === s && e.progress > r)) && ((n = i - 1), (t = i), (a = 1 + s));
              }),
              (r = e.progress || 0),
              (void 0 === n && void 0 === t) || e.gl.setProgress(n, t, a, s);
          }),
          n("setTransition", (n, t) => {
            "gl" !== e.params.effect || o || (s = t > 0 && !e.params.cssMode);
          }),
          n("destroy", () => {
